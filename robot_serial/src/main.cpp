...
uint8_t aim_switch[9]= {0,1,0,1,1,1,1,0,0};
...
void depthcallback(const geometry_msgs::PoseStamped &msg) {
    exchangestation _exchange{};
    _exchange.x = (float) msg.pose.position.x;
    _exchange.y = (float) msg.pose.position.y;
    _exchange.z = (float) msg.pose.position.z;
...
void chassisCallback(const geometry_msgs::Twist::ConstPtr &msg) {
    chassis c{
            .vx = (int16_t) (msg->linear.x * 2008) + c_rc.vx,
            .vy = (int16_t) (msg->linear.y * 2008) + c_rc.vy,
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber depth_sub = nh.subscribe("/robot/depth", 1, depthcallback);
